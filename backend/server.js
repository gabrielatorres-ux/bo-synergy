...
app.post('/api/consultas', async (req, res) => {
  const { paciente_id, fecha, motivo, alergias, cabeza, cuello, torax, abdomen, espalda,
    extremidades_superiores, extremidades_inferiores, ojos_oidos_garganta, causa,
    impresion_diagnostica, medicamentos, receta, cie10 } = req.body;
  try {
    const result = await queryRun(
...
app.put('/api/consultas/:id', async (req, res) => {
  const { id } = req.params;
  const { fecha, motivo, alergias, cabeza, cuello, torax, abdomen, espalda,
    extremidades_superiores, extremidades_inferiores, ojos_oidos_garganta, causa,
    impresion_diagnostica, medicamentos, receta, cie10 } = req.body;
  try {
    await queryRun(
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, user: userWithoutPassword, message: `Bienvenido ${result.nombre}` });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/usuarios', async (req, res) => {
  ...
  try {
    const result = await queryRun(
      'INSERT INTO usuarios (num_empleado, nombre, rol, password, fecha_registro) VALUES ($1, $2, $3, $4, NOW()) RETURNING id',
...
app.get('/api/estadisticas', async (req, res) => {
  try {
    const [totalPacientes, totalConsultas, totalEMI, totalEMP, totalEMR, totalVulnerabilidad] = await Promise.all([
      query('SELECT COUNT(*) as total FROM pacientes'),
      query('SELECT COUNT(*) as total FROM consultas'),
      query('SELECT COUNT(*) as total FROM emi'),
      query('SELECT COUNT(*) as total FROM emp'),
      query('SELECT COUNT(*) as total FROM emr'),
      query('SELECT COUNT(*) as total FROM vulnerabilidad'),
    ]);
    res.json({
      totalPacientes: parseInt(totalPacientes.rows[0].total),
      totalConsultas: parseInt(totalConsultas.rows[0].total),
      totalEMI: parseInt(totalEMI.rows[0].total) || 0,
      totalEMP: parseInt(totalEMP.rows[0].total) || 0,
      totalEMR: parseInt(totalEMR.rows[0].total) || 0,
      totalVulnerabilidad: parseInt(totalVulnerabilidad.rows[0].total) || 0,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/top-motivos', async (req, res) => {
  try {
    const result = await query(
      `SELECT motivo, COUNT(*) as count FROM consultas WHERE motivo IS NOT NULL AND motivo != '' 
...
app.get('/api/top-areas', async (req, res) => {
  try {
    const result = await query(
      `SELECT p.area, COUNT(c.id) as count FROM consultas c 
       JOIN pacientes p ON c.paciente_id = p.id 
       WHERE p.area IS NOT NULL AND p.area != '' 
       GROUP BY p.area ORDER BY count DESC LIMIT 5`
    );
    res.json(result.rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  ...
  try {
    const result = await query(
      `SELECT TO_CHAR(fecha, 'YYYY-MM') as mes, COUNT(*) as count 
       FROM consultas GROUP BY mes ORDER BY mes DESC LIMIT 12`
    );
    res.json(result.rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.get('/api/pacientes-por-area', async (req, res) => {
  try {
    const result = await query(
      `SELECT area, COUNT(*) as count FROM pacientes 
